...
if (isset($_SESSION["t"])) {
    ...
    if (empty($fname)) {
        ...
        Database::iud("UPDATE `teacher_qualifications` SET `name`='" . $quli . "' WHERE `id` = '" . $_SESSION["t"]["teacher_qualifications_id"] . "'");
        ...
        if ($nr == 1) {
            ...
            Database::iud("UPDATE `teacher` SET `fname`='" . $fname . "',`lname`='" . $lname . "',
            `full_name`='" . $fullname . "',`dob`='" . $dob . "',`phone`='" . $phone . "',
            `mobile`='" . $mobile . "',`address_id`='" . $add["id"] . "',
            `nic`='" . $nic . "' WHERE `email`='" . $_SESSION["t"]["email"] . "' ");

            echo "Teacher profile updated || Teacher Address updated ";
        } else {
            Database::iud("UPDATE `teacher` SET `fname`='" . $fname . "',`lname`='" . $lname . "',
            `full_name`='" . $fullname . "',`dob`='" . $dob . "',`phone`='" . $phone . "',
            `mobile`='" . $mobile . "',
        ...
        if (!in_array($file_extention, $allowed_image_extention)) {
            ...
            $resultProfileImg = Database::search("SELECT * FROM `profile_img` WHERE `user_email`='" . $_SESSION["t"]["email"] . "'  ");
            $pror = $resultProfileImg->num_rows;

            if ($pror == 1) {


                Database::iud("UPDATE `profile_img` SET `code`='" . $filename . "' WHERE `user_email`='" . $_SESSION["t"]["email"] . "'  ");
